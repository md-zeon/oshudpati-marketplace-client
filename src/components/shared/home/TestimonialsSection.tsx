    ...
    <section className="relative py-10">
      <div className="mx-auto max-w-7xl">
        <div className="mx-auto flex max-w-md flex-col items-center justify-center gap-4">
          <h2 className="font-bold text-3xl tracking-tighter lg:text-4xl text-foreground text-center">
...
function TestimonialCard({
  ...
  return (
    <figure
      className={cn(
        "w-full max-w-sm rounded-3xl border border-border-default bg-card p-6 shadow-lg shadow-black/5 transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl",
        className,
      )}
      {...props}
    >
        ...
        {[...Array(5)].map((_, i) => (
          <Star
            ...
            className={`w-3.5 h-3.5 ${
              i < rating ? "text-accent-500 fill-accent-500" : "text-border-default"
            }`}
          />
        ))}
      </div>
      ...
      <figcaption className="mt-4 flex items-center gap-3">
        <Avatar className="size-10 rounded-full border border-border-default">
          <AvatarImage alt={name} src={image || ""} />
          <AvatarFallback className="bg-brand-50 text-brand-700 text-xs font-bold">
            {initials || "U"}
          </AvatarFallback>
        </Avatar>
        <div className="flex flex-col">
          <cite className="font-semibold not-italic leading-5 tracking-tight text-foreground text-sm">
            {name}
          </cite>
