...
export default function AdminCategories() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res?.success) {
        ...
        setDialogOpen(false);
        fetchCategories();
      } else toast.error(res?.message || "Failed to save", { id: tid });
    } catch {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (res?.success) {
      ...
      fetchCategories();
    } else toast.error(res?.message || "Failed to delete", { id: tid });
  };
        ...
        {categories.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 stagger-children">
            {categories.map((cat) => (
              <div
                key={cat.id}
                className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-md transition-all duration-300 hover:-translate-y-0.5"
              >
